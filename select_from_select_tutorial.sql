...
select name, continent, population from world x where (select max(population) from world y where y.continent = x.continent) <= 25000000


-- 10 Some countries have populations more than three times that of any of their neighbours (in the same continent). Give the countries and continents.

select name, continent from world x where population > (select max(3*population) from world y where y.continent = x.continent and y.name != x.name)
